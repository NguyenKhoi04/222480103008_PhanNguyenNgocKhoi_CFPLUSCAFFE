...
class _FragmentHistoryState extends State<FragmentHistory> {
  ...
  late Stream<List<Map<String, dynamic>>> donHangStream;

 @override
void initState() {
  super.initState();

  donHangStream = supabase
      .from('orders')
      .stream(primaryKey: ['id'])
      .eq('user_id', supabase.auth.currentUser!.id) // nếu cần lọc user
  ...
  Color getTrangThaiColor(String status) {
    switch (status) {
      case "unpaid":
    ...
    return Column(
      children: [
        ...
        Expanded(
          ...
          hinhAnh: product['image_url'] ?? "",

          soLuong: item['quantity'] ?? 1,

          tongTien: (item['total_amount'] ?? 0).toInt(),

          trangThai:
              hienThiTrangThai(item['payment_status'] ?? ""),

          size: item['size'] ?? "",

          mucDa: item['ice_level'] ?? "",

          ghiChu: item['note'] ?? "",

          tenBan: item['table_id']?.toString() ?? "",
                    ...
                    child: ListTile(
                      ...
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          image: donHang.hinhAnh.isNotEmpty
                              ? DecorationImage(
